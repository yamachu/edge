...
function determineVersion() {
    if (process && process.versions && process.versions['electron']) {
        for (var i in versionMapElectron) {
            if (process.versions['electron'].match(versionMapElectron[i][0])) {
                return versionMapElectron[i][1];
            }
        }
    } else {
...
exports.func = function(language, options) {
    ...
    if (typeof language !== 'string') {
        ...
        var scopedCompilerName = '@yamachu/edge-' + language.toLowerCase();
        var tryScoped = false;
        var compiler;
        try {
        ...
        if (tryScoped) {
            try {
                compiler = require(scopedCompilerName);
            }
            catch(e) {
